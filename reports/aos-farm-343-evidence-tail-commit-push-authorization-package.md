# AOS-FARM.343 Пакет авторизации push (Evidence Tail)

- **created commit SHA**: 0a7e61fdd17b906588d4715f6bc44f3732ec5982
- **commit message**: docs: preserve evidence tail reports
- **committed file count**: 154
- **committed file list**:
...
- **origin/dev before push**: ae8059e921fa5be767c9637186b4b4e8a0cc8312
- **ahead/behind state**: dev ahead of origin/dev by 1, behind by 0
- **excluded file count**: 42
- **statement that push is not yet authorized**: ВАЖНО: Пуш (push) еще НЕ авторизован.
- **statement that Human Push Authorization is required before push**: Перед выполнением пуша требуется явная авторизация (Human Push Authorization).
- **statement that force push, tag push, release, and production use are not authorized**: Force push, отправка тегов, создание релизов и использование в production строго запрещены.
