...
class HumanResultAcceptanceChecker:
    ...
    def _validate_data(self, decision: dict) -> dict:
        # Check required decision presence
        human_decision = decision.get("human_decision")
        if not human_decision:
            return {"status": "BLOCKED", "reason": "missing human_decision"}
        ...
        if human_decision == "ACCEPT_RESULT":
            ...
            if "UNKNOWN" in str(decision):
                # Basic check for unknown required fields
                return {"status": "BLOCKED", "reason": "ACCEPT_RESULT with UNKNOWN required fields"}
                
            if "NOT_RUN" in str(decision):
            ...
            if not decision.get("rejection_reason"):
                return {"status": "BLOCKED", "reason": "REJECT_RESULT without rejection_reason"}
