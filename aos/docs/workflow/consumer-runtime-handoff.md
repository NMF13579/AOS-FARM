...
**Temporary Files:** All temporary command outputs, interim logs, and scratchpad items generated by agents must be stored in a local-only `/.aos-tmp/` directory. Target projects should create and use `/.aos-tmp/` at their root. The `.aos-tmp/` directory must **never** be placed inside the `aos/` product folder itself, nor should it ever contain Evidence, reports, or approval checkpoints.
